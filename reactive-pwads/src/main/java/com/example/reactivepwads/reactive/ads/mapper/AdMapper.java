...
public class AdMapper {
    ...
    public Mono<BasicAd> basicAdDtoToBasicAd(AdDto dto) throws UserNotFoundException {
        return ReactiveSecurityContextHolder
                ...
                .flatMap(userDetails -> userRepository.findByUsername(userDetails.getUsername()).map(user -> mapDtoToBasicAd((BasicAdDto) dto, user)));
    }
    ...
    public Mono<CarAd> carAdDtoToCarAd(AdDto dto) throws UserNotFoundException {
        return ReactiveSecurityContextHolder
                .getContext()
                .map(context -> (UserDetails) context.getAuthentication().getPrincipal())
                .flatMap(userDetails -> userRepository.findByUsername(userDetails.getUsername()).map(user -> mapDtoToBasicAd((CarAdDto) dto, user)));
    }
